...
def vca_argument_spec():
    return dict(
        ...
        password=dict(
            type="str", aliases=["pass", "passwd"], required=True, no_log=True
        ),
        org=dict(),
        service_id=dict(),
        instance_id=dict(),
        ...
        service_type=dict(
            default=DEFAULT_SERVICE_TYPE, choices=list(SERVICE_MAP.keys())
        ),
        vdc_name=dict(),
...
class VcaAnsibleModule(AnsibleModule):
    def __init__(self, *args, **kwargs):
        ...
        if not HAS_PYVCLOUD:
            self.fail(missing_required_lib("pyvcloud"), exception=PYVCLOUD_IMP_ERR)
    ...
    def login_vcd(self):
        ...
        self.vca.login(
            token=self.vca.token, org=org, org_url=self.vca.vcloud_session.org_url
        )
...
def _validate_module(module):
    if not HAS_PYVCLOUD:
        module.fail_json(
            msg=missing_required_lib("pyvcloud"), exception=PYVCLOUD_IMP_ERR
        )
    ...
    if service_type == "vca":
        for arg in VCA_REQ_ARGS:
            if module.params.get(arg) is None:
                module.fail_json(
                    msg="argument %s is mandatory when service type " "is vca" % arg
                )
    ...
    if service_type == "vchs":
        for arg in VCHS_REQ_ARGS:
            if module.params.get(arg) is None:
                module.fail_json(
                    msg="argument %s is mandatory when service type " "is vchs" % arg
                )
    ...
    if service_type == "vcd":
        for arg in VCD_REQ_ARGS:
            if module.params.get(arg) is None:
                module.fail_json(
                    msg="argument %s is mandatory when service type " "is vcd" % arg
                )
...
def _vca_login(vca, password, instance):
    ...
    if not vca.login_to_instance_sso(instance=instance):
        ...
        raise VcaError(
            "Login to Instance failed: Seems like instance_id provided "
...
def _vchs_login(vca, password, service, org):
    ...
    if not vca.login_to_org(service, org):
        raise VcaError(
            "Failed to login to org, Please check the orgname",
            error=vca.response.content,
        )
...
def _vcd_login(vca, password, org):
    ...
    if not vca.login(password=password, org=org):
        raise VcaError(
            "Login Failed: Please check username or password " "or host parameters"
        )

    if not vca.login(password=password, org=org):
        raise VcaError("Failed to get the token", error=vca.response.content)
